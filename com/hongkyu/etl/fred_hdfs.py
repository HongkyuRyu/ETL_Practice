...
import os
import time
from com.hongkyu.etl.us_states import US_STATES
...
class Fred2Hdfs(object):
    ...
    def __init__(self):
        ...
        path = 'C:\\Users\\minsu\\PycharmProjects\\ETL_Practice\\com\\hongkyu\\etl'
        os.chdir(path)
    ...
    def getFredDF(self, freq, state, str_search):
        ...
        mask_df = (df_fred_col.title == str_search_text) \
                & (df_fred_col.frequency_start == str_search_text) \
        ...
        if not df_fred_col.empty:
            ...
            df_etl_col['frequency_start'] = df_fred_col.iloc[0].frequency_start
            df_etl_col['units_start'] = df_fred_col.iloc[0].units_short
            df_etl_col['seasonal_adjustments_short'] = df_fred_col.iloc[0].seasonal_adjustment_short
    ...
    def appendCsvHdfs(self, filename, df_data):
